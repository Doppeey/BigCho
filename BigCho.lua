...
function getWDmg(Target)
    ...
    if not Cho.W:IsLearned() then return 0

    end

    if not Cho.W:IsReady() then return 0
...
function getQDmg(Target)
    ...
    if not Cho.Q:IsLearned() then return 0

    end

    if not Cho.Q:IsReady() then return 0
...
function Cho.OnDrawDamage(target, dmgList)
    if not Menu.Get("Draw.ComboDamage") then return end
